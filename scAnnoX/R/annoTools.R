...
sciBetAnno <- function(obj.seu, ref.obj, ref.ctype = NULL, ...) {
    test.df <- GetAssayData(obj.seu, layer = "data") %>%
        as.data.frame() %>%
        t()
    ref.df <- GetAssayData(ref.obj, layer = "data") %>%
        as.data.frame() %>%
        t()
    if (!is.null(ref.ctype)) {
        label.ref <- ref.obj@meta.data[, ref.ctype]
...
scmapAnno <- function(obj.seu, ref.obj, ref.ctype = NULL, layer.data = c("data", "count"), ...) {
    system.file(package = "scAnnoX") %>%
        file.path(., "scmap-master") %>%
        load_all(.)

    ref.expr <- GetAssayData(ref.obj, assay = "RNA",layer = layer.data[1]) %>% as.matrix()
    if (!is.null(ref.ctype)) {
        label.ref <- ref.obj@meta.data[, ref.ctype]
    } else {
        label.ref <- Idents(ref.obj)
    ...
    test.expr <- GetAssayData(obj.seu,assay = "RNA", layer = layer.data[1]) %>% as.matrix()
...
chetahAnno <- function(obj.seu, ref.obj, ref.ctype = NULL, layer.data = "data", ...) {
    test.expr <- GetAssayData(obj.seu, layer = layer.data) %>% as.matrix()
    sce.test <- SingleCellExperiment(assays = list(counts = test.expr))
    if (layer.data == "count") sce.test <- scater::logNormCounts(sce.test)

    ref.expr <- GetAssayData(ref.obj, layer = layer.data) %>% as.matrix()
    if (!is.null(ref.ctype)) {
        label.ref <- ref.obj@meta.data[, ref.ctype]
    } else {
        label.ref <- Idents(ref.obj)
    ...
    if (layer.data == "count") sce.ref <- scater::logNormCounts(sce.ref)
...
scinaAnno <- function(obj.seu, marker.lst, ...) {
    pred.res <- SCINA::SCINA(
        obj.seu %>% GetAssayData(., layer = "data") %>% as.matrix(),
        marker.lst,
        max_iter = 1000,
        convergence_n = 12,
